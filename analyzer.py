...
class PreAnalyzer:
    ...
    def analyze_version(self, tag: str, previous_tag: Optional[str] = None) -> str:
        """
        Perform pre-analysis on a specific version tag
        Returns LLM-friendly analysis report
        """
        self.findings.clear()
        ...
        commit_shas = self.repo.get_commits_for_tag(tag, previous_tag)
        self.findings.append(f"Total commits analyzed: {len(commit_shas)}")
        ...
        self.repo.build_contributor_profiles(commit_shas)
        
        # 1. Analyze metadata (files, packages, build configs)
        self._analyze_metadata(commit_shas)
        
        # 2. Analyze contributor trust
        self._analyze_contributors()
        
        # 3. Analyze code and library changes
        self._analyze_changes(commit_shas)
        
        return '\n'.join(self.findings)
    
    def _analyze_metadata(self, commit_shas: List[str]) -> None:
    ...
    def _analyze_contributors(self) -> None:
    ...
    def _analyze_changes(self, commit_shas: List[str]) -> None:
...
def main():
    ...
    if len(sys.argv) < 3:
        print("Usage: python analyzer.py <repo_path> <version_tag> [previous_tag]")
        print("Example: python analyzer.py ./mongoose 8.19.1 8.19.0")
        sys.exit(1)
    ...
    report = analyzer.analyze_version(version_tag, previous_tag)
    ...
    # Save to file
    output_file = f"pre_analysis_{version_tag.replace('/', '_')}.txt"
